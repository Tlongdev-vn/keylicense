...
local SAVE_FILE_NAME = "Tlongkey2System.txt" -- File lưu trạng thái key
...
local Translations = {
    VI = {
        Title = "TLONG KEY SYSTEM - Nasi rendang  HUB",
...
local function LaunchMainScript()
    task.spawn(function()
        local success, result = pcall(function()
            return loadstring(game:HttpGet("https://raw.githubusercontent.com/JualNasiRendang/loader/refs/heads/main/main.lua"))()
        end)
        if not success then
            warn("[Nasi rendang Hub Error]:", result)
        end
    end)
end
